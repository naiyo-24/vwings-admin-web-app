...
const Support = () => {
  ...
  const handleSubmitTicket = async (e) => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/helpcenter/create`, {
        method: 'POST',
          ...
          <AnimatePresence>
            {status === 'success' && (
              <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px', color: '#4ade80', padding: '16px', background: 'rgba(74, 222, 128, 0.1)', border: '1px solid rgba(74, 222, 128, 0.3)', borderRadius: '12px', marginBottom: '8px' }}>
                  <CheckCircle size={24} color="#4ade80" />
                  <strong style={{ color: '#4ade80' }}>Your support ticket has been submitted successfully!</strong>
                </div>
              </motion.div>
            )}
            {status === 'error' && (
              <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }}>
                <div style={{ color: '#f87171', padding: '16px', background: 'rgba(248, 113, 113, 0.1)', border: '1px solid rgba(248, 113, 113, 0.3)', borderRadius: '12px', marginBottom: '8px' }}>
